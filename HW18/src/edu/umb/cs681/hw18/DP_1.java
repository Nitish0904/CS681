...
//			System.out.println(matrix);
			...
			List<String> values=matrix.stream().parallel().skip(1).map(row -> row.get(total_value)).collect(Collectors.toList());
//			System.out.println(values);
			
			//converting the column string values to float values
			List<Float> v = values.stream().parallel().filter(str -> !str.isEmpty()).map(Float::parseFloat).collect(Collectors.toList());
//			System.out.println(v);
			
			OptionalDouble min= v.stream().parallel().mapToDouble(Float::intValue).min();
			
			System.out.println("Minimum value of Property in Boston: "+min.getAsDouble());
			
			OptionalDouble max= v.stream().parallel().mapToDouble(Float::intValue).max();
			
			System.out.println("Max value of Property in Boston: "+max.getAsDouble());
			
			OptionalDouble avg= v.stream().parallel().mapToDouble(Float::intValue).average();
